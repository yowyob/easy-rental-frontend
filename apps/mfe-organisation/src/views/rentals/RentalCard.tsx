...
import { Calendar, MapPin, ArrowUpRight, Info, Banknote, User } from 'lucide-react';

export const RentalCard = ({ rental, onView, t }: any) => {
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      default: return 'bg-red-50 text-red-700 border-red-100 dark:bg-red-900/20 dark:text-red-400';
    }
  };
    ...
    <div className="bg-white dark:bg-[#1a1d2d] rounded-[2.5rem] p-6 md:p-8 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-lg transition-all group flex flex-col h-full text-left relative overflow-hidden">
      ...
      <div className="space-y-4 mb-8 pt-4 border-t border-slate-50 dark:border-slate-800">
        <div className="flex items-center gap-3 text-[11px] font-black text-slate-500 dark:text-slate-400 italic">
          <Calendar size={14} className="text-[#0528d6] dark:text-blue-400" /> 
          <span>{new Date(rental.startDate).toLocaleDateString()} — {new Date(rental.endDate).toLocaleDateString()}</span>
        </div>
        <div className="flex items-center gap-3 text-[11px] font-black text-slate-500 dark:text-slate-400 italic">
          <Banknote size={14} className="text-[#0528d6] dark:text-blue-400" /> 
          <span className="text-slate-900 dark:text-white text-sm">{rental.totalAmount?.toLocaleString()} <span className="text-[9px] opacity-50">{t.common.currency}</span></span>
        </div>
      </div>
