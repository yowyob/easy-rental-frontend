  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 pb-20 text-left animate-in fade-in duration-500">
      <div className="flex flex-col items-start justify-between gap-4 px-1 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-2 text-[10px] font-medium text-slate-400">
            Historique conservé en base — lectures indépendantes de la console organisation.
          </p>
        </div>
        <button
          type="button"
      ...
      <NotificationsFilterBar
        ...
        labels={{ all: t.common.all, unread: 'Non lues', alerts: 'Alertes' }}
      />

      {filteredNotifs.length === 0 ? (
        <NotificationsEmptyState message={t.common.noData} />
      ) : (
          ...
          {filteredNotifs.map((n) => {
            ...
            return (
              <NotificationCard
                ...
                readLabel={read ? 'Lu (agence)' : undefined}
              />
            );
          })}
        </div>
